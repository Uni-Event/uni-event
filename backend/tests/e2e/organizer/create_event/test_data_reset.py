...
# Completează câmpurile de dată/ora în formatul UI
def _fill_dates_ui_format(page: Page):
    ...
    if dt_inputs.count() >= 2:
        ...
        dt_inputs.nth(0).type(start_str, delay=25)
        dt_inputs.nth(0).blur()

        dt_inputs.nth(1).click()
        dt_inputs.nth(1).press("Control+A")
        dt_inputs.nth(1).type(end_str, delay=25)
...
def _set_location_via_map(page: Page):
    ...
    if leaflet.count() > 0:
        ...
        leaflet.click(position={"x": 240, "y": 170})
...
# Test pentru resetarea formularului prin UI
def test_reset_event_ui(page: Page):
    ...
    title.fill(f"Test Event {datetime.now().strftime('%H%M%S')}")
    ...
    # Apasă butonul Resetează și confirmă
    def _on_dialog(dialog):
        dialog.accept()

    page.once("dialog", _on_dialog)

    reset_btn = page.get_by_role("button", name="Resetează")
    ...
    expect(title).to_have_value("", timeout=15000)
    if seats.count() > 0:
        expect(seats).to_have_value("", timeout=15000)
    expect(desc).to_have_value("", timeout=15000)
