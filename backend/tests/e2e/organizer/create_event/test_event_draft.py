...
def _fill_dates_ui_format(page: Page):
    ...
    if dt_inputs.count() >= 2:
        ...
        # fallback: format datetime-local
...
# Apasă Salvează Ciornă și verifică că s-a declanșat submit
def _click_draft_and_expect_submit(page: Page, modal_heading):
    draft_btn = page.get_by_role("button", name="Salvează Ciornă")
    expect(draft_btn).to_be_visible(timeout=15000)
    ...
    confirm = page.locator("text=/ciornă|draft|salvat|salvare/i").first
    expect(confirm).to_be_visible(timeout=3000)

# Test pentru salvarea ca ciornă prin UI
def test_save_draft_event_ui(page: Page):
    ...
    # Draft
    _click_draft_and_expect_submit(page, modal_heading)
